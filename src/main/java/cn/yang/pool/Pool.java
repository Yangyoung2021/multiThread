...
public class Pool {
    ...
    //连接对象数组
    private final TestConnection[] connections;
    //连接状态 0表示空闲，1表示占用中
    private final AtomicIntegerArray states;

    public Pool(int poolSize) {
        this.poolSize = poolSize;
        this.connections = new TestConnection[poolSize];
        this.states = new AtomicIntegerArray(poolSize);
        for (int i = 0; i < poolSize; i++) {
            //初始化连接对象
            connections[i] = new TestConnection();
        }
    }
    ...
    public Connection connection() {
        while (true) {
            ...
            synchronized (this) {
                try {
                    System.out.println(Thread.currentThread().getName() + "连接失败，进入等待");
                    this.wait();
